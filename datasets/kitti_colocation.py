...
class ColocationKITTIDataset(Dataset):
    '''
    ...
    def __init__(self,config,split,data_augmentation=True):
        super(ColocationKITTIDataset,self).__init__()
        self.config = config
        ...
        self.prepare_kitti_ply_colocation(split)
        print(f"Data size for phase {split}: {len(self.files)}")

    def prepare_kitti_ply_colocation(self, split='train'):
    ...
    def __getitem__(self, idx):
        ...
        xyz_cmpl = []
        for t_tmp in t_cmpl:
            xyz_cmpl.append(self._get_xyz(drive, t_tmp))
