...
from lib.utils import load_obj, natural_key, _exhaustive_hash
from datasets.kitti import KITTIDataset
from datasets.kitti_colocation import ColocationKITTIDataset
from datasets.nuscenes_colocation import ColocationNuscenesDataset
...
def collate_fn_descriptor_colocation(list_data, config, neighborhood_limits):
    # batched_points_list = []
    # batched_features_list = []
    # batched_lengths_list = []
    assert len(list_data) == 1
    
    for ind, (xyz_batch_input, feats_batch, group, index, finest_flag, central_distance, list_M, sample) in enumerate(list_data):
        batched_points_list = xyz_batch_input
        batched_features_list = feats_batch
        batched_lengths_list = [len(xyz) for xyz in xyz_batch_input]
    ...
    if not config.use_group_circle_loss:
        index_hash = _exhaustive_hash(torch.split(index, tuple(group.tolist())), len(batched_features))
    else:
    ...
    dict_inputs = {
        ...
        'list_M': list_M,
        'center_pcd': torch.from_numpy(xyz_batch_input[0]).float(),
        'group': group,
        'index': index,
        'finest_flag': finest_flag,
...
def collate_fn_descriptor(list_data, config, neighborhood_limits):
    ...
    # print(batched_points_list, flush=True)
...
def get_datasets(config):
    ...
    elif (config.dataset == 'kitti_colocation'):
        train_set = ColocationKITTIDataset(config,'train',data_augmentation=True)
        val_set = KITTIDataset(config,'val',data_augmentation=False)
        benchmark_set = KITTIDataset(config, 'test',data_augmentation=False)
    elif (config.dataset == 'nuscenes_colocation'):
        train_set = ColocationNuscenesDataset(config,'train',data_augmentation=True)
        val_set = NUSCENESDataset(config,'val',data_augmentation=False)
        benchmark_set = NUSCENESDataset(config, 'test',data_augmentation=False)
    elif(config.dataset=='nuscenes'):
        train_set = NUSCENESDataset(config,'train',data_augmentation=True)
        val_set = NUSCENESDataset(config,'val',data_augmentation=False)
        benchmark_set = NUSCENESDataset(config, 'test',data_augmentation=False)
    else:
...
def get_dataloader(dataset, batch_size=1, num_workers=4, shuffle=True, neighborhood_limits=None, colocation=False):
    if neighborhood_limits is None:
        if colocation:
            neighborhood_limits = calibrate_neighbors(dataset, dataset.config, collate_fn=collate_fn_descriptor_colocation)
        else:
    ...
    if colocation:
        dataloader = torch.utils.data.DataLoader(
            ...
            collate_fn=partial(collate_fn_descriptor_colocation, config=dataset.config, neighborhood_limits=neighborhood_limits),
            drop_last=False
        )
    else:
        dataloader = torch.utils.data.DataLoader(
